...
game_state = {
  init = function()
    score, entities, bullets, enemies, ebullets = 0, {}, {}, {}, {}
    sublists = { bullet = bullets, enemy = enemies, ebullet = ebullets }
    init_player()
    p.weapon_lv = 3
    if p.archetype == 3 then p.shield = 1 end
    shake_t, shake_mag, hit_flash_t, death_flash_t, death_timer, boss_spawned, boss_ent, hit_stop_t, reached_hi_score_this_run = 0, 0, 0, 0, 0, false, nil, 0, false
  end,
  update = function()
    update_stars()
    if not p.dead then
